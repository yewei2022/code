...
# df = pd.read_excel(data_dir +"高影响路段桩号处理.xlsx",sheet_name = 0,
...
# #仅过滤S107
# df1 = df[df['督办路段点位编码'] != "S107"]
# # 重新拼接处理过经纬度的S107
# dff= pd.concat([df1, S107],axis = 0, ignore_index = True) 
# dff.columns = ['road_id','lon','lat','alti','pile_id','province','city','county',
#               'road','road_type','weather']
# #写入文件
# dff.to_csv(data_dir+"桩号.txt",
...
# #写入文件
# sta_pick.to_csv(data_dir+"road_sta_pick.txt",
#               index = False,sep=' ',na_rep=32700)

#%% branch2 根据站点筛选浓雾年均日数

# sta_pick = pd.read_table(data_dir+"road_sta_pick.txt",sep = "\s+")
...
# road_fog2.to_csv(data_dir+"road_sta_fogdays.txt",
...
# road_fog1 = road_fog.drop(['lon','lat'],axis=1)
# road_fog2 = road_fog1.reset_index(drop = False)
# road_fog2.to_csv(data_dir+"road_sta_tmp0days.txt",
#                   index = False,columns = ['road_id','pile_id',
#                 'pile_lat','pile_lon','sta','sta_lat', 'sta_lon',
#                 'alti','dist','days'],sep=' ',na_rep=32700)
