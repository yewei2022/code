...
for j in range(0,7):
    ...
    a0.loc[a0["站名"].isin(["二洞","梓潼马鸣云峰"]),"站名"]= " "
    ...
    leftlon, rightlon, lowerlat, upperlat = (104.8,105.6,31.3,32.) #!!!换县
    ...
    if j==3:
        ...
        pre_grid = inverse_distance_to_grid(
            lon, lat, pre, lon_gridmesh, lat_gridmesh, r = 0.26, 
            min_neighbors = 0.05)
    elif j==5:
        pre_grid = inverse_distance_to_grid(
    ...
    #法二给出间隔，出等差数列  #!!! 改  
    interval = [0.1, 0.1, 0.1, 5, 0.01, 0.01, 0.005]
    if j == 3:
        value_min =  250
        value_max =  800
        ...
        levels1 = levels[40:-15]    
    else:
        # 法一 给出数量，出等差数列
        nlevels = 101
        levels = np.linspace(value_min,value_max,nlevels)
        levels1 = levels[35:-28]
        
        # # 法二 给出间隔，出等差数列
        # levels = np.arange(value_min,value_max,0.025)    
        # levels1 = levels[50:-50]
