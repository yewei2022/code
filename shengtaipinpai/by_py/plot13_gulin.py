# -*- coding: utf-8 -*-
"""
Created on Fri Nov  3 11:00:05 2023
...
for j in range(0,7):
    ...
    # 再去掉一些地名
    a0.loc[a0["站名"].isin(["二郎","椒园","金兰凤田","永乐永吉",
                          "茅溪园区","德耀福来", "长田","龙山稻香",
                          "德耀红光","德耀红光"]),"站名"] = " "
    ...
    leftlon, rightlon, lowerlat, upperlat = (105.5,106.4,27.6,28.4) #!!!古蔺
    ...
    if j==4 or j==5 :
        ...
        pre_grid = inverse_distance_to_grid(
            lon, lat, pre, lon_gridmesh, lat_gridmesh, r = 0.23, 
            min_neighbors = 0.025)
    ...
    #降水
        ...
        value_min = 300
        value_max = 490
        levels = np.arange(value_min,value_max,0.1)    
        levels1 = levels
    #温度
    elif 0<=j < 3:
        levels1 = levels[5:-10]
    ...
    # 标记站点名 去掉站名有以下字符的行 如果只是像前面那样变成空 依然占绘图空间
    a0 = a0[~a0['站名'].isin(["土城", "丹桂", "双沙" , "茅溪柏腊",
                          "太平"])]
    # 添加站点名    
    ...
    # 保存
    plt.savefig(pic_dir+sheet_en[i]+"\\py1\\"+sheet_en[i]+str(j)
